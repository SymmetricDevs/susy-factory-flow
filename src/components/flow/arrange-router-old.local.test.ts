...
import { arrangeBoard, type ArrangeCard, type ArrangeWire } from "@/lib/board-arrange-old.local";
...
it("arranges a capture and routes it", { timeout: 600000 }, () => {
  ...
  const judge = (positions: ReadonlyMap<string, { x: number; y: number }>): number => {
    ...
    return totalCrossings(solveGridRoutes(obs, reqs));
  };
  const t0 = performance.now();
